...
$(document).ready(function(){
	...
	$(".cntChangeNon").click(function(){
		...
		if(result == false){
		 ...
		 var store_code = $(this).next().next().next().val();
		 $totalprice =  $(this).parent().next().children(":eq(0)");
	    ...
	    <div class="tab-content">
	        ...
	        <c:if test="${not empty cartlist}">
	    	 <!-- -일반배송 회원일때 --> 
			  ...
			  <c:if test="${empty nonBookVoList}">
			   <br><br>
			  "2장바구니에 담긴 상품이 없습니다."
			  </c:if>
			  
			  <c:if test="${not empty nonBookVoList}">
				<form action="${pageContext.request.contextPath}/soomin/order/orderStep0.do" method="get">  
             	<br><br>	          
					...
					<tr>
						<td style="padding-up:20px;">
							<div class="checkbox"><!-- 개별 체크박스 -->
								...
								<input type="checkbox" onclick = "totalpriceNon1()" class="chBoxNon" name="NonMemberBook_no" value="${nonBookVo.book_no }">
						...
						<td style="padding-left:20px">
							...
							<input type="button" value="변경" class="cntChangeNon"><!--일반배송 비회원 -->
							<input type="hidden" value="${nonBookVo.getBook_saleprice() }">
							<input type="hidden" value="${nonBookVo.book_no }">
							<input type="hidden" value="${nonBookVo.store_code }">
	    	...
	    	<c:if test="${empty cartbarolist }">
	    	 <br><br>
	    	"3장바구니에 담긴 상품이 없습니다."
	    	</c:if>
	    	<c:if test="${not empty cartbarolist}">
	    		<div class="allCheck"><!-- 전체선택:지니일반배송옆 -->
	    			<br><br>
				...
				<table border id="sangpumjungbo">
	    			...
	    			<c:forEach var="quickPickCartList" items="${cartbarolist }"><!-- 회원 바로드림 -->
					<tr>
						<td style="padding-up:20px;">
							<div class="checkbox"><!-- 개별 체크박스 -->
	    	...
	    	<c:if test="${empty nonBookVoList}">
	    	 <br><br>
	    	 "4장바구니에 담긴 상품이 없습니다.:비회원 장바구니둘다 아무것도 없을때"
	    	</c:if>
	    	
	    	<c:if test="${not empty nonBookVoList}"><!-- 비회원 장바구니 -->
	    	<c:forEach var="nonBookVo" items="${nonBookVoList}" >
	    		<c:if test="${nonBookVo.store_code != 'online'}" >
	    			<c:set var="store_code" value="1"/>
	    		</c:if>	
	    	</c:forEach>
	    	
	    	<c:if test="${ store_code!=1}">
	    	  <br><br>
	    	 "복사) 장바구니에 담긴 상품이 없습니다.:바로드림만 없을때 "
	    	</c:if>
	    	<c:if test="${ store_code==1}">
					...
					<tr>
						<td style="padding-up:20px;">
							...
							<script>
								...
								/* function totalpriceNon11111(){
						     		var total = 0;
						     		$("input[name='NonMemberBook_no']:checked").each(function(e){
						     			var value = parseInt($(this).parent().parent().next().next().next().children(":eq(0)").html());
						     			alert("totalpriceNon11111의 function : " + value);
						     			total += value;
						     		})
	     ...
	     <%--배송 유의 사항  --%>
              <table id="lastT">
              	<tr>
              		<th id="lastTfirstTh">배송 유의사항 및 <br>이용 혜택 안내</th>
              		<th>
              			<a href=#none id="show" 
